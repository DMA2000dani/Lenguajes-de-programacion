...
# noms dels paràmetres
        llistaNoms = self.visit(l[3])
# valors dels paràmetres
        llistaValors = self.parametres[nom]

# Diccionari de variables nou per aquesta funció
        variablesNoves = {}
        if llistaNoms is not None:
...
# Execució de les instruccions de la funció
        self.visit(l[6])
# Eliminar les variables que hi havien
        self.pila.pop()
        return
    ...
    def visitParametreVariable(self, ctx):
        ...
        for i in self.variables:
            if trobat == 0 and i == text:
                print(self.variables[text], end=' ')
                trobat = 1
        if trobat == 0:
            print(text, end=' ')
    ...
    def visitExprDiv(self, ctx):
        l = list(ctx.getChildren())
        if self.visit(l[2]) == 0:
            print('\nError, Estas tractant de fer una divisio entre per 0 \n')
            exit()
        else:
